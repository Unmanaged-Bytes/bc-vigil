...
from bc_vigil.integrity import cron_builder, scheduler
...
def preview_schedule(
    ...
    return request.app.state.templates.TemplateResponse(
        request, "schedules/_preview.html",
...
def toggle_schedule(schedule_id: int, session: Session = Depends(get_session)):
    ...
    session.commit()
    scheduler.sync_schedule(session, schedule_id)
    return RedirectResponse(f"/targets/{schedule.target_id}", status_code=303)
